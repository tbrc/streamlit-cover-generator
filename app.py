...
def generate_cover_image(market):
    prompt = (
        f"Generate a cover image in 16:9 ratio, 1200 pixels wide, on the {market}. No text needed. Only need real images, avoid illustrational images. Do not combine multiple visual elements into a single image, need one single image without merging 2-3 images. Avoid divided layouts, side-by-side compositions, split screens, no layered imagery, no surreal or conceptual blending—keep the image simple, realistic, and immediately understandable."
    )
